...
class _PatientAllExercisesPageWidgetState
  ...
  Future<Null> readExerciseResults() async {
    for (var i = 0; i < exerciseNumberOfWeeks; i++) {
        ...
        await FirebaseFirestore.instance
          ...
          for (var item in value.docs) {
            ...
            if (exerciseDate
              ...
              // ตรวจสอบว่าวันที่มีการออกกำลังกายนั้นอยู่ก่อนถึงวันนี้หรือไม่
              // ถ้าใช่ให้ทำการเพิ่มสถานะการออกกำลังกายไปยัง exerciseCompletions
              ...
              await FirebaseFirestore.instance
                ...
                // ตรวจสอบว่ารูปภาพท่าออกกำลังกายนั้นมีอยู่ใน exerciseImagesModel แล้วหรือไม่
                // หากยังไม่มีให้เพิ่มรูปภาพนั้นไปยัง exerciseImagesModel
      ...
      // เมื่ออ่านผลการออกกำลังกายและท่าออกกำลังกายในสัปดาห์นั้นเสร็จแล้ว
      // ให้จัดเรียงรูปภาพของสัปดาห์นั้นใน exerciseImagesModel โดยเรียงจากท่าออกกำลังกายที่มีการออกกำลังกายก่อนจนถึงสุดท้าย
      ...
      // หลังจากเพิ่มข้อมูลผลการออกกำลังกายและรูปภาพท่าออกกำลังกายของสัปดาห์นั้นเรียบร้อยแล้ว
      // ให้ทำการเปลี่ยนค่า firstDateOfTheWeek และ clear exerciseCompletions
      // เพื่อที่จะหาข้อมูลผลการออกกำลังกายและรูปภาพท่าออกกำลังกายของสัปดาห์ถัดไป
      setState(() {
        firstDateOfTheWeek = firstDateOfTheWeek.add(Duration(days: 7));
        exerciseCompletions.clear();
      });
    }
    // หากอ่านผลการออกกำลังกายและท่าออกกำลังกายในแต่ละสัปดาห์เสร็จเรียบร้อยแล้ว
    // ให้เปลี่ยนค่า readDataIsFinished เป็น true
    setState(() {
  ...
  Future<void> refreshPage() async {
    if (internetIsConnected == true) {
      exerciseDates.clear();
      exerciseCompletions.clear();
      exerciseResultsInEachWeekModel.clear();
      await readExerciseDates();
    }
  }
  ...
  Future<Null> goToPatientThatWeekExercisesPageWidget(
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PatientThatWeekExercisesPageWidget(
